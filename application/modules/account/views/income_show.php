...
<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12" align="center" style="padding-top: 20px;">
    ...
    <div align="center" class="panel panel-default" style="width: 70%; margin-top: 10px; border: 1px solid black; box-shadow: 2px 2px 10px 10px #000000; opacity: 0.8">
...
<script>
    $(document).ready(function() {
        ...
        $('.example').DataTable({
         ...
         $('#delete').click(function () {
            ...
            $.ajax({
                ...
                method: 'post',
                data: {id: id, tblname: 'tbl_income'},
                success: function () {
                    $('#delete_nc').modal('hide');
                    window.location= base_url+"account/account/income_show"; 
                }
            })
        });
        ...
        $('#update_c').submit(function (e) {
            ...
            var date= $.trim($('#datepick').val());
            var unit= $.trim($('#unit').val());
            var rate= $.trim($('#rate').val());
            var amount= $.trim($('#amount').val());
            if(party_name === ''||category_name === ''||name === ''||date === ''||amount === ''){
                if(category_name === ''){
                    $('#category_name_error').html('Please Enter name');
                ...
                if(name === ''){
                    $('#name_error').html('Please Enter Particular\' name');
                    $('#name').css("border-color", "red"); 
                }
                if(date === ''){
                    $('#datepick_error').html('Please Choose Date');
                    $('#datepick').css("border-color", "red"); 
                }
                if(amount === ''){
                    $('#amount_error').html('Please Enter name');
                    $('#amount').css("border-color", "red"); 
                }
            }else{
                $.ajax({
                    url: base_url+'account/account/income_update',
                    method: 'post',
                    data: {id: id, party_name: party_name, category_name: category_name, name: name, date: date, unit: unit, rate: rate, amount: amount},
                    success: function () {
                        $('#update').modal('hide');
                        window.location= base_url+"account/account/income_show";                        
                    }
    ...
    function updateIDReserver(ida){ 
        $.ajax({
                ...
                method: 'post',
                data: {id: ida, tblname: 'tbl_income', page : 'income_updat'},
