...
<script>
    $(document).ready(function() {
        var base_url="<?php echo base_url();?>";
         $('#delete').click(function () {
            ...
            $.ajax({
                ...
                method: 'post',
                data: {id: id, tblname: 'tbl_user'},
                success: function () {
                    $('#delete_nc').modal('hide');
                    window.location= base_url+"account/account/user_show"; 
                }
            })
        });
        ...
        $('#update_c').submit(function (e) {
            ...
            if(user_name === '' || password === '' || user_type === '' || status === ''){
                if(user_name === ''){
                ...
                if(status === ''){
                    $('#status_error').html('Please Choose Status');
                    $('#status').css("border-color", "red"); 
                }
            }else{
                $.ajax({
                    url: base_url+'account/account/user_update',
                    method: 'post',
                    data: {id: id, user_name: user_name, password: password, user_type: user_type, status: status},
                    success: function () {
                        $('#update').modal('hide');
                        window.location= base_url+"account/account/user_show";                        
                    }
    ...
    function updateIDReserver(ida){ 
        $.ajax({
                ...
                method: 'post',
                data: {id: ida, tblname: 'tbl_user', page : 'user_updat'},
